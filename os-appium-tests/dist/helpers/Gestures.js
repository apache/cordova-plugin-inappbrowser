...
class Gestures {
     ...
     * @param {number} amount
     */
    static checkIfDisplayedWithScrollDown(element, maxScrolls, amount = 0, driver) {
     ...
     */
    static swipeDown(percentage = 1, driver) {
        this.swipeOnPercentage(this._calculateXY(SWIPE_DIRECTION.down.start, percentage), this._calculateXY(SWIPE_DIRECTION.down.end, percentage), driver);
    }
    /**
     ...
     * @param {number} percentage from 0 - 1
     */
    static swipeUp(percentage = 1, driver) {
        this.swipeOnPercentage(this._calculateXY(SWIPE_DIRECTION.up.start, percentage), this._calculateXY(SWIPE_DIRECTION.up.end, percentage), driver);
    }
    /**
     ...
     * @param {number} percentage from 0 - 1
     */
    static swipeLeft(percentage = 1, driver) {
        this.swipeOnPercentage(this._calculateXY(SWIPE_DIRECTION.left.start, percentage), this._calculateXY(SWIPE_DIRECTION.left.end, percentage), driver);
    }
    /**
     ...
     * @param {number} percentage from 0 - 1
     */
    static swipeRight(percentage = 1, driver) {
        this.swipeOnPercentage(this._calculateXY(SWIPE_DIRECTION.right.start, percentage), this._calculateXY(SWIPE_DIRECTION.right.end, percentage), driver);
    }
    /**
     ...
     * </pre>
     */
    static swipeOnPercentage(from, to, driver) {
        ...
        this.swipe(pressOptions, moveToScreenCoordinates, driver);
    }
    /**
     ...
     * </pre>
     */
    static swipe(from, to, driver, pause = 1000) {
        driver.touchPerform([{
                action: 'press',
                options: from,
            }, {
    ...
    static twoFingerPress(touchDuration, driver) {
    ...
    static pinchZoomIn(driver, pause = 1000) {
        SCREEN_SIZE = SCREEN_SIZE || driver.getWindowRect();
        driver.multiTouchPerform([
            [
    ...
    static pinchZoomOut(driver, pause = 1000) {
        driver.multiTouchPerform([
            [
     ...
     * @private
     */
    static _getPercentDeviceCoords(screenSize, coordinates) {
        return {
