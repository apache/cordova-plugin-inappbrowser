...
- (void)setToolbarFlatColor:(NSString*)color
{
	...
	self.navBg = [[UIView alloc]initWithFrame:CGRectMake(toolFrame.origin.x, toolFrame.origin.y, toolFrame.size.width, toolFrame.size.height)];
	...
	if ([statusBar respondsToSelector:@selector(setBackgroundColor:)])
		statusBar.backgroundColor = [UIColor colorWithHexValue:flatColor andAlpha:1.0f];
}
...
- (void)setToolbarGradientColor:(NSString*)color1 : (NSString *) color2 : (NSString *) alpha1 : (NSString *) alpha2
{
	...
	if (color1 != nil && color2 != nil)
	{
		...
		// Setup gradient layer for status bar
		gradientStatus.frame = [[UIApplication sharedApplication] statusBarFrame];
		gradientStatus.colors = [NSArray arrayWithObjects:(id)[[UIColor colorWithHexValue:[self convertHexString:color1] andAlpha:gradientalpha1] CGColor], (id)[[UIColor colorWithHexValue:[self convertHexString:color2] andAlpha:gradientalpha2] CGColor], nil];
		gradientStatus.startPoint = CGPointMake(0, 1);
		gradientStatus.endPoint = CGPointMake(1, 1);
		
		if ([statusBar respondsToSelector:@selector(setBackgroundColor:)])
			[statusBar.layer insertSublayer:gradientStatus atIndex:0];
		
		// Setup gradient layer for toolbar
		gradientToolbar.frame = self.toolbar.frame;
		gradientToolbar.colors = gradientStatus.colors;
		gradientToolbar.startPoint = gradientStatus.startPoint;
...
- (void)viewDidLoad
{
	...
	gradientToolbar = [CAGradientLayer layer];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
	...
	gradientStatus.hidden = YES;
	statusBar.backgroundColor = [UIColor clearColor];
}

- (UIStatusBarStyle)preferredStatusBarStyle
{
	if (_browserOptions.statusbarstylelight == YES)
