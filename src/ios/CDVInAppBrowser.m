...
- (void)close:(CDVInvokedUrlCommand*)command
{
    ...
    [self.inAppBrowserViewController minimize];
}
...
- (void)open:(CDVInvokedUrlCommand*)command
{
    ...
    if (url != nil) {
        ...
        if ([url length] == 0) {
            absoluteUrl = nil;
            self.inAppBrowserViewController.navigationDelegate = self.inAppBrowserViewController.tempNavigationDelegate;
        }
...
- (void)browserMinimize
{
    ...
    // Set navigationDelegate to nil to ensure no callbacks are received from it.
    self.inAppBrowserViewController.tempNavigationDelegate = self.inAppBrowserViewController.navigationDelegate;
    self.inAppBrowserViewController.navigationDelegate = nil;
    // Don't recycle the ViewController since it may be consuming a lot of memory.
    // Also - this is required for the PDF/User-Agent bug work-around.
    //self.inAppBrowserViewController = nil;
...
- (void)minimize
{
    NSLog(@"MINIMIZING IAB");
    [CDVUserAgentUtil releaseLock:&_userAgentLockToken];
    self.currentURL = nil;
    
    if ((self.navigationDelegate != nil) && [self.navigationDelegate respondsToSelector:@selector(browserExit)]) {
        [self.navigationDelegate browserMinimize];
    }
