    ...
    //if is an app store link, let the system handle it, otherwise it fails to load it
        ...
        if (_navigationBlockingPolicies != nil){
            ...
            for(NSDictionary *policy in _navigationBlockingPolicies) {
                ...
                NSLogDebug(@"ShouldLoad,policy %@", policy);
                if(policy.count < 1) {
                    continue;
                }
                ...
                for(NSString *key in policy) {
                    ...
                    NSLogDebug(@"ShouldLoad,policy,key %@", key);
                    NSLogDebug(@"ShouldLoad,policy,value %@", value);
                    ...
                    NSLogDebug(@"ShouldLoad,policy,eventValue %@", eventValue);
                    ...
                    if ([regex firstMatchInString:eventValue options:0 range:NSMakeRange(0, [eventValue length])] == nil) {
                        policyFulfilled = NO;
                        NSLogDebug(@"ShouldLoad,policyFulfilled NO");
                    }
        ...
        if(_shouldBlock) {
            ...
            CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                                                            messageAsDictionary:@{@"type":@"navigation_blocked", @"url":[url absoluteString]}];
            [pluginResult setKeepCallback:[NSNumber numberWithBool:YES]];
